...
import co.za.fenya.ehr.config.KafkaProperties;
...
class EmployeeportalKafkaResourceIT {
    ...
    void setup() {
        ...
        EmployeeportalKafkaResource kafkaResource = new EmployeeportalKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/employeeportal-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/employeeportal-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
